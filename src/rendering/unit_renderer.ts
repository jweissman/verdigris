...
export class UnitRenderer {
  ...
  shouldRenderUnit(unit: Unit): boolean {
    ...
    // Skip dead units in some views
    if (unit.state === 'dead' && unit.hp <= 0) {
      // Could make this configurable per view
  ...
  getAnimationFrame(unit: Unit, animationTime: number): number {
    if (unit.state === 'dead') {
      ...
      // Idle animation - cycle between frames 0 and 1
   ...
   */
  drawHealthBar(ctx: CanvasRenderingContext2D, unit: Unit, screenX: number, screenY: number) {
