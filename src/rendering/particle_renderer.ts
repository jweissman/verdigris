...
export class ParticleRenderer {
  ...
  private renderLeafParticle(
    ...
    if (leafSprite) {
      ...
      const rotation =
        (particle.pos.x * 0.1 + particle.lifetime * 0.02) % (Math.PI * 2);
      ctx.rotate(rotation);
  ...
  private renderLightningParticle(
    ctx: CanvasRenderingContext2D,
    particle: any,
    x: number,
    y: number,
