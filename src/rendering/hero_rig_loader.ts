...
export class HeroRigLoader {
  static loadFromJSON(): HeroRig {
    ...
    if (heroRigData.defaultPose) {
      Object.entries(heroRigData.defaultPose).forEach(([partName, partData]: [string, any]) => {
    ...
    if (heroRigData.anchors) {
      Object.entries(heroRigData.anchors).forEach(([anchorName, anchorData]: [string, any]) => {
    ...
    if (heroRigData.weapons) {
      Object.entries(heroRigData.weapons).forEach(([weaponType, weaponData]: [string, any]) => {
    ...
    if (heroRigData.animations) {
      Object.entries(heroRigData.animations).forEach(([animName, animData]: [string, any]) => {
        rig.addAnimation({
          name: animName,
  ...
  static loadFromJSONData(data: any): HeroRig {
    ...
    if (data.defaultPose) {
      Object.entries(data.defaultPose).forEach(([partName, partData]: [string, any]) => {
        rig.setPart(partName as any, partData);
      });
    }
    
    if (data.anchors) {
      Object.entries(data.anchors).forEach(([anchorName, anchorData]: [string, any]) => {
        rig.setAnchor(anchorName as any, anchorData);
      });
    }
    
    if (data.weapons) {
      Object.entries(data.weapons).forEach(([weaponType, weaponData]: [string, any]) => {
        rig.setWeaponConfig(weaponType as any, weaponData);
      });
    }
    
    if (data.animations) {
      Object.entries(data.animations).forEach(([animName, animData]: [string, any]) => {
