export class CursorRenderer {
  ...
  render(ctx: CanvasRenderingContext2D, toIsometric: (x: number, y: number) => { x: number; y: number }) {
    // Draw hover cursor
    if (this.hoverPos) {
      ...
      // Draw diamond shape for tile hover
      ctx.beginPath();
    ...
    // Draw click marker (fades out over time)
    if (this.clickPos) {
      const elapsed = Date.now() - this.clickTime;
      if (elapsed < 500) { // Show for 500ms
