...
export class AbilitiesOptimized extends Rule {
  ...
  private static precompiledAbilities = new Map<string, {
    trigger?: Function;
    target?: Function;
    ability: Ability;
  }>();
  ...
  constructor() {
    ...
    // Compile abilities once at startup
  ...
  execute(context: TickContext): QueuedCommand[] {
    ...
    // Process units using indices only
    for (const idx of activeIndices) {
      // Skip dead units using array access
      ...
      // Skip units with only combat abilities (handled by specialized rules)
      ...
      for (const abilityName of abilities) {
        // Skip combat abilities
        if (abilityName === 'melee' || abilityName === 'ranged') continue;
        
        const precompiled = AbilitiesOptimized.precompiledAbilities.get(abilityName);
        ...
        if (lastTick !== undefined && currentTick - lastTick < ability.cooldown) {
          continue;
        }
        ...
        // Evaluate trigger using indices
        if (precompiled.trigger) {
          try {
            // Create minimal proxy-like object for trigger evaluation
            const unitData = {
              id: unitId,
              pos: { x: posX[idx], y: posY[idx] },
              hp: hp[idx],
              team: team[idx] === 1 ? 'friendly' : team[idx] === 2 ? 'hostile' : 'neutral',
        ...
        // Find target using indices if needed
  ...
  private queueAbilityEffects(
    ...
    for (const effect of ability.effects) {
      // Create minimal command without creating proxies
      switch (effect.type) {
        case 'damage':
          if (targetIdx !== -1) {
            this.commands.push({
        ...
        // For other effects, queue a generic ability_effect command
        default:
          this.commands.push({
            type: 'ability_effect',
            params: {
