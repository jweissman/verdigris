...
export class GauntletGame extends Game {
  ...
  // Gauntlet sequence from VISION.md
  private stages = [
    {
      name: "Castle Gates",
      background: "castle",
  ...
  bootstrap() {
    ...
    // Start with expanded tactical space - "double-wide battle strips"
    this.sim.fieldWidth = this.stages[0].width;
    this.sim.fieldHeight = this.stages[0].height;
    
    // Use isometric view for dramatic effect
  ...
  private loadStage(stageIndex: number) {
    ...
    // Set background for this stage
    this.sim.sceneBackground = stage.background;
    
    // Spawn hero at left side of each stage
    this.spawnHero();
    
    // Create stage-appropriate enemies
    this.spawnStageEnemies(stage);
    
    // Visual effect for stage transition
  ...
  private spawnStageEnemies(stage: any) {
    ...
    for (let i = 0; i < enemyCount; i++) {
      ...
      // Distribute enemies across the stage
    ...
    console.log(`⚔️  Spawned ${enemyCount} enemies: ${stage.enemies.join(", ")}`);
  }
  ...
  private createStageTransitionEffect() {
    // Create dramatic particle effects for stage transitions
    const stage = this.stages[this.currentStage];
  ...
  private displayStageInfo() {
    const stage = this.stages[this.currentStage];
    console.log(`\\n📍 Current: ${stage.name}`);
    console.log(`📊 Progress: Stage ${this.currentStage + 1}/${this.stages.length}`);
    console.log(`🎯 Objective: Clear all enemies to advance`);
  }
  
  private checkStageCompletion() {
    const enemies = this.sim.units.filter(u => 
      u.team === "hostile" && u.hp > 0 && u.state !== "dead"
    );
    
    // Stage complete when all enemies defeated
    if (enemies.length === 0) {
      this.completeStage();
      return true;
    }
  ...
  private createVictoryEffect() {
    ...
    // Golden victory particles around hero
    for (let i = 0; i < 16; i++) {
