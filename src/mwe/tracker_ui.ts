...
class TrackerUI {
  ...
  private init(): void {
    ...
    const patternInput = document.getElementById('pattern-dsl') as HTMLInputElement;
    
    if (playBtn) playBtn.addEventListener('click', () => this.play());
    if (stopBtn) stopBtn.addEventListener('click', () => this.stop());
    if (clearBtn) clearBtn.addEventListener('click', () => this.clear());
    if (loadBtn) loadBtn.addEventListener('click', () => this.loadPatternFromDSL());
    if (loadSongBtn) loadSongBtn.addEventListener('click', () => this.loadSongFromLibrary());
  ...
  private loadSongData(songData: any): void {
    // Convert song data to tracker pattern format
    this.currentPattern = {
      id: songData.id,
      name: songData.name,
  ...
  private updatePatternDisplay(): void {
    // This would update the visual grid - simplified for now
    this.updateStatus(`Pattern: ${this.currentPattern?.name || 'None'} (${this.currentPattern?.length || 0} steps)`);
  }
