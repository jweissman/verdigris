...
export class PlantDefenseGame extends Game {
  ...
  private plantBush() {
    ...
    if (currentTick - lastPlant < 20) {
      console.log(
        `Plant on cooldown: ${20 - (currentTick - lastPlant)} ticks remaining`,
      );
      return;
    }
  ...
  private moveFarmer(dx: number, dy: number) {
    ...
    const newX = Math.max(
      0,
      Math.min(this.sim.fieldWidth - 1, farmer.pos.x + dx),
    );
    const newY = Math.max(
      0,
      Math.min(this.sim.fieldHeight - 1, farmer.pos.y + dy),
    );
  ...
  private restartWave() {
    const toRemove = this.sim.units.filter(
      (u) => u.team === "hostile" || u.type === "bush",
    );
  ...
  update() {
    ...
    const deadEnemies = this.sim.units.filter(
      (u) => u.team === "hostile" && u.state === "dead",
    );
