...
export class Tournament2v2 {
  ...
  runAll(
    ...
    onProgress?: (
      current: number,
      total: number,
      stats?: { ticks?: number; simTimeMs?: number },
    ) => void,
    ...
    for (const team1 of teams) {
      ...
      if (
        percentComplete - lastRankingReport >= 10 ||
        currentMatch - (lastRankingReport * totalMatchups) / 100 >= 5000
      ) {
        console.log(
  ...
  private runMatch(
    team1: [string, string],
    team2: [string, string],
  ): MatchResult & { stats?: { ticks: number; simTimeMs: number } } {
