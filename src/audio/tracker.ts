...
export class Tracker {
  ...
  // Convert tracker pattern to engine pattern
      ...
      const notes: (Note | null)[] = track.notes.map(trackerNote => {
        ...
        return {
          ...
          slide: trackerNote.effect?.startsWith('S') ? 
            parseInt(trackerNote.effect.slice(1)) : undefined
        };
      });
  ...
  static parsePatternDSL(dsl: string): TrackerPattern {
    // Parse format like:
    // "bass: C2 ... E2 ... / lead: uarp(C4) / drums: kick ... snare ..."
