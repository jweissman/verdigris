...
export class Soundboard {
  ...
  getPadNames(): string[] {
    return this.config.pads.map((pad, i) => (pad ? pad.name : `empty-${i}`));
  }
  ...
  createChord(root: number, type: "major" | "minor" = "major"): SoundPad {
    ...
    const frequencies = intervals.map(
      (interval) => root * Math.pow(2, interval / 12),
    );
  ...
  importConfig(configJson: string): boolean {
    try {
      ...
      if (
        !config.pads ||
        !Array.isArray(config.pads) ||
        config.pads.length !== 16
      ) {
        return false;
      }
