...
export class SynthEngine {
  ...
  constructor() {
    // Don't initialize audio context until needed (for testing)
  }
  ...
  // Add a track with patterns
  addTrack(track: Track): number {
    this.tracks.push(track);
    return this.tracks.length - 1;
  }
  
  // Core synthesis - generate audio buffer
  synthesize(note: Note, voice: VoicePreset): Float32Array {
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      // Apply envelope
      const env = this.calculateEnvelope(i / sampleRate, duration, voice.envelope);
      sample *= env;
  ...
  // Sequencer - schedule and play patterns
  play(): void {
  ...
  private getCurrentPattern(track: Track, step: number): Pattern | null {
    // Simple: use first pattern for now
  ...
  private scheduleNote(note: Note, track: Track, time: number): void {
    ...
    // Get voice for this track's channel
  ...
  private getVoiceForChannel(channel: number): VoicePreset {
    // Default voices per channel
    const voices: VoicePreset[] = [
      { waveform: 'sine', envelope: { attack: 0.01, decay: 0.1, sustain: 0.7, release: 0.2 } },
      { waveform: 'square', envelope: { attack: 0.001, decay: 0.05, sustain: 0.3, release: 0.1 } },
      { waveform: 'saw', envelope: { attack: 0.01, decay: 0.2, sustain: 0.5, release: 0.3 } },
      { waveform: 'triangle', envelope: { attack: 0.001, decay: 0.01, sustain: 0.9, release: 0.05 } }
    ];
    
    return voices[channel % voices.length];
  }
  
  // Tracker-style pattern editing
  setNote(trackIndex: number, patternIndex: number, stepIndex: number, note: Note | null): void {
...
export class NoteUtils {
  private static noteNames = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
  ...
  // Create "uarp" pattern (ascending arpeggio)
  ...
  // Create "darp" pattern (descending arpeggio)
  static darp(root: string, steps: number = 4): Note[] {
    return this.uarp(root, steps).reverse();
  }
  
  // Parse tracker notation like "C-4 64 1" (note velocity gate)
