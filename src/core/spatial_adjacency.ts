...
export class SpatialAdjacency {
  ...
  processAdjacent(
    ...
    for (const [cellKey, indices] of this.grid) {
      ...
      // Process pairs within same cell
      for (let i = 0; i < indices.length; i++) {
        ...
        // Check adjacent cells (only right/down/diagonals to avoid duplicates)
        const neighbors = [
          [cellX + 1, cellY],     // right
          [cellX, cellY + 1],     // down  
          [cellX + 1, cellY + 1], // down-right
          [cellX - 1, cellY + 1]  // down-left
        ];
        ...
        for (const [nx, ny] of neighbors) {
          ...
          // Check all units in neighbor cell
  ...
  getMeleePairs(arrays: any): Array<[number, number]> {
    ...
    this.processAdjacent(arrays, MELEE_RANGE_SQ, (idxA, idxB, distSq) => {
      ...
      if (arrays.team[idxA] !== arrays.team[idxB]) {
        // Check if either can attack (not dead, has hp)
  ...
  getKnockbackPairs(arrays: any): Array<[number, number]> {
    ...
    this.processAdjacent(arrays, KNOCKBACK_RANGE_SQ, (idxA, idxB, distSq) => {
      // Both units need mass for knockback
