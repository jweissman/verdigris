...
export interface VisualEffect {
  id: string;
  type: 'flash' | 'screen-tint' | 'particle-burst' | 'screen-shake' | 'zoom-pulse';
...
export class CameraEffects {
  ...
  addShake(intensity: number, duration: number, frequency: number = 20): string {
  ...
  addEffect(type: VisualEffect['type'], intensity: number, duration: number, params?: any): string {
  ...
  addFlash(color: string, intensity: number = 1.0, duration: number = 10): string {
    return this.addEffect('flash', intensity, duration, { color });
  }

  addScreenTint(color: string, intensity: number = 0.5, duration: number = 30): string {
    return this.addEffect('screen-tint', intensity, duration, { color });
  }

  addParticleBurst(pos: { x: number; y: number }, color: string, count: number = 20, duration: number = 60): string {
    return this.addEffect('particle-burst', 1.0, duration, { pos, color, count });
  }
  ...
  // Get current shake offset for camera positioning
  getShakeOffset(): { x: number; y: number } {
    ...
    for (const shake of this.shakes.values()) {
      if (shake.remainingTime > 0) {
        ...
        // Use frequency to create rapid oscillation
  ...
  // Get current visual effects for rendering
  getActiveEffects(): VisualEffect[] {
    return Array.from(this.effects.values()).filter(effect => effect.remainingTime > 0);
  }
  ...
  // Clear all effects (for scene transitions etc)
  clear(): void {
    this.shakes.clear();
    this.effects.clear();
  }
  ...
  heroicLeapEffect(startPos: { x: number; y: number }, endPos: { x: number; y: number }): void {
