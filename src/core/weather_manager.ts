...
export class WeatherManager {
  public weather: {
    current: "clear" | "rain" | "snow" | "sandstorm" | "fog" | "storm" | "leaves" | "lightning";
    duration: number;
    intensity: number;
  };
  ...
  setWeather(
    type: "clear" | "rain" | "snow" | "sandstorm" | "fog" | "storm" | "leaves" | "lightning",
    duration: number,
  ...
  processWeatherCommand(command: string, ...args: any[]): void {
    switch (command) {
      case "rain":
        const duration = typeof args[0] === 'string' ? parseInt(args[0]) : (args[0] || 200);
        const intensity = typeof args[1] === 'string' ? parseFloat(args[1]) : (args[1] || 0.5);
        this.setWeather("rain", duration, intensity);
        break;
      case "storm":
        const stormDuration = typeof args[0] === 'string' ? parseInt(args[0]) : (args[0] || 300);
        const stormIntensity = typeof args[1] === 'string' ? parseFloat(args[1]) : (args[1] || 0.8);
