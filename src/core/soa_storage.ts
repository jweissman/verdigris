...
export class SoAStorage {
  // Core position arrays for physics
  posX: Float32Array;
  posY: Float32Array;
  
  // Velocity/movement arrays
  velX: Float32Array;
  velY: Float32Array;
  ...
  // Active tracking (0=inactive/deleted, 1=active)
  active: Uint8Array;
  
  // Metadata indices - maps to separate storage
  ...
  // Current capacity and active count
  capacity: number;
  activeCount: number;
  
  // ID to index mapping for lookups
  ...
  addUnit(unit: any): number {
    // Find first available slot or grow if needed
  ...
  applyMovement(fieldWidth: number, fieldHeight: number): void {
    // Vectorized addition of intended movement to position
  ...
  detectCollisions(radius: number = 1): Array<[number, number]> {
    ...
    // Spatial hashing for O(n) average case instead of O(n^2)
    const grid = new Map<string, number[]>();
    
    // Hash all active units
    for (let i = 0; i < this.capacity; i++) {
      if (this.active[i] === 0) continue;
  ...
  private grow(): void {
    ...
    this.intendedMoveX = this.growArray(this.intendedMoveX, newCapacity, Float32Array);
    this.intendedMoveY = this.growArray(this.intendedMoveY, newCapacity, Float32Array);
