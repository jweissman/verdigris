...
// Pure mathematical waveform generators
export class Oscillator {
  constructor(public type: 'sine' | 'square' | 'saw' | 'triangle' = 'sine') {}
  
  generate(frequency: number, numSamples: number, sampleRate: number = 44100): Float32Array {
  ...
  generateWithEnvelope(
    ...
    // Attack phase
    for (let i = 0; i < attackSamples && sampleIndex < numSamples; i++, sampleIndex++) {
      const attackLevel = i / attackSamples;
      output[sampleIndex] = raw[sampleIndex] * attackLevel;
    }
    
    // Decay phase
    for (let i = 0; i < decaySamples && sampleIndex < numSamples; i++, sampleIndex++) {
      const decayLevel = 1 - (1 - envelope.sustain) * (i / decaySamples);
      output[sampleIndex] = raw[sampleIndex] * decayLevel;
    }
    
    // Sustain phase
    for (let i = 0; i < sustainSamples && sampleIndex < numSamples; i++, sampleIndex++) {
      output[sampleIndex] = raw[sampleIndex] * envelope.sustain;
    }
    
    // Release phase
    for (let i = 0; i < releaseSamples && sampleIndex < numSamples; i++, sampleIndex++) {
...
// Preset voices with rich characteristics
...
export class Channel {
  ...
  generate(note: string | number, numSamples: number): Float32Array {
    ...
    if (this.voice === 'choir') {
      ...
      const detune1 = this.oscillator.generate(freq * 1.01, numSamples, this.sampleRate);
      const detune2 = this.oscillator.generate(freq * 0.99, numSamples, this.sampleRate);
  ...
  private applyReverb(samples: Float32Array, wetness: number): Float32Array {
    ...
    // Add delayed copies that extend into the tail
    for (let i = 0; i < samples.length; i++) {
      for (let tap = 1; tap <= 8; tap++) {
        const delayIndex = i - delaySamples * tap;
        if (delayIndex >= 0) {
          const reverbSample = samples[delayIndex] * Math.pow(decay, tap) * wetness;
          output[i] += reverbSample;
          
          // Also feed forward to create tail effect
...
export class Synthbox {
  ...
  play(pattern: string): void {
    // Parse patterns like "choir: C#m Amaj7 Bm / lead: uarp uarp rest uarp"
