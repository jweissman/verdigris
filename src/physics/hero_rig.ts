...
export class HeroRig {
  ...
  private createDefaultSkeleton() {
    ...
    const rightArm = this.createBone('right_arm', rightShoulder, 2, Math.PI / 4);
    const rightForearm = this.createBone('right_forearm', rightArm, 1.5, Math.PI / 8);
    ...
    const leftArm = this.createBone('left_arm', leftShoulder, 2, 3 * Math.PI / 4);
    const leftForearm = this.createBone('left_forearm', leftArm, 1.5, -Math.PI / 8);
    const leftHand = this.createBone('left_hand', leftForearm, 0.5, 0);
    
    // Legs
    const hips = this.createBone('hips', null, 0.5, Math.PI / 2);
    const rightThigh = this.createBone('right_thigh', hips, 2, Math.PI / 2 + Math.PI / 8);
    const rightShin = this.createBone('right_shin', rightThigh, 2, 0);
    const rightFoot = this.createBone('right_foot', rightShin, 0.8, Math.PI / 2);
    
    const leftThigh = this.createBone('left_thigh', hips, 2, Math.PI / 2 - Math.PI / 8);
  ...
  private createBone(name: string, parent: Bone | null, length: number, angle: number): Bone {
    const bone: Bone = {
      name,
      start: { x: 0, y: 0 },
      end: { x: 0, y: 0 },
  ...
  private updateBoneFK(bone: Bone, parentEnd: Vec2) {
    ...
    const adjustedAngle = this.facing === 'left' && bone.name.includes('right') 
      ? Math.PI - bone.angle 
      : bone.angle;
  ...
  public update(deltaTime: number = 1) {
    ...
    for (const [boneName, targetTransform] of currentFrame.bones) {
      ...
      if (nextTransform) {
        // Lerp between frames
        bone.angle = this.lerp(targetTransform.angle, nextTransform.angle, easedT);
      } else {
  ...
  public reachFor(boneName: string, target: Vec2): boolean {
    ...
    // Get chain of bones from root to target bone
    const chain: Bone[] = [];
    ...
    // Extract points and lengths for FABRIK
    ...
    if (reached) {
      // Update bone angles from solved positions
  ...
  public getRenderData(): any {
    // Convert skeleton to render-friendly format
    const bones: any[] = [];
