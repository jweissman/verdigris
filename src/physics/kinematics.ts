...
export class KinematicSolver {
  ...
  private static satisfyConstraint(
...
export class GrapplingRope {
  ...
  constructor(grapplerPos: Vec2, targetPos: Vec2, maxLength: number = 10) {
  ...
  update(newGrapplerPos: Vec2, newTargetPos: Vec2): void {
    ...
    this.chain.nodes[this.chain.nodes.length - 1].position = {
      ...newTargetPos,
    };
  ...
  applyPull(force: number): void {
    ...
    const dx =
      this.chain.nodes[0].position.x - this.chain.nodes[lastNode].position.x;
    const dy =
      this.chain.nodes[0].position.y - this.chain.nodes[lastNode].position.y;
    const dist = Math.sqrt(dx * dx + dy * dy);

    if (dist > 0) {
      KinematicSolver.applyForceToChain(this.chain, lastNode, {
        x: (dx / dist) * force,
        y: (dy / dist) * force,
      });
    }
