  ...
  execute(unitId: string | null, params: CommandParams): void {
    ...
    if (targetEnemy) {
      ...
      if (enemies.length > 0) {
        ...
        enemies.sort((a, b) => {
          const distA =
            Math.abs(a.pos.x - hero.pos.x) + Math.abs(a.pos.y - hero.pos.y);
          const distB =
            Math.abs(b.pos.x - hero.pos.x) + Math.abs(b.pos.y - hero.pos.y);
          return distA - distB;
        });
    ...
    if (dashToEnemy && damage > 0) {
      ...
      const hitEnemies = this.sim.units.filter(
        (u) =>
          u.team !== hero.team &&
          u.hp > 0 &&
    ...
    console.log(
      `[Dash] Hero ${unitId} dashes from (${startX}, ${startY}) to (${targetX}, ${targetY})`,
    );
  }
}
