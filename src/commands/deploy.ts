    ...
    if (x !== undefined && y !== undefined) {
      ...
      if (deployerUnit) {
        const enemies = this.sim.units.filter(
          (u) => u.team !== deployerUnit.team && u.hp > 0,
        );
        if (enemies.length > 0) {
          const closestEnemy = enemies.reduce((closest, enemy) => {
            const dist1 = Math.sqrt(
              Math.pow(deployerUnit.pos.x - enemy.pos.x, 2) +
                Math.pow(deployerUnit.pos.y - enemy.pos.y, 2),
            );
            const dist2 = Math.sqrt(
              Math.pow(deployerUnit.pos.x - closest.pos.x, 2) +
                Math.pow(deployerUnit.pos.y - closest.pos.y, 2),
            );
