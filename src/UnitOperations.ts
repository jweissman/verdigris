...
export class UnitOperations {
  static move(unit: Unit, deltaTime: number = 1, sim?: any): Unit {
    ...
    if (
      sim &&
      typeof sim.fieldWidth === "number" &&
      typeof sim.fieldHeight === "number"
    ) {
  ...
  static hunt(unit: Unit, sim: any): Unit {
    ...
    let closestDist =
      Math.abs(closest.pos.x - unit.pos.x) +
      Math.abs(closest.pos.y - unit.pos.y);
    for (const hostile of hostiles) {
      const dist =
        Math.abs(hostile.pos.x - unit.pos.x) +
        Math.abs(hostile.pos.y - unit.pos.y);
      if (dist < closestDist) {
        closest = hostile;
        closestDist = dist;
      }
