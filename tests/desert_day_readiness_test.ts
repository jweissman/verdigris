...
describe('Desert Day Visual Readiness Checklist', () => {
  ...
  it('✓ Combat scenario demonstration', () => {
    const { sim } = setupDesertScene();
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (i % 10 === 0) {
        const activeGrapples = sim.projectiles.filter(p => p.type === 'grapple').length;
        const grappledUnits = sim.units.filter(u => u.meta?.grappled).length;
      }
    }
