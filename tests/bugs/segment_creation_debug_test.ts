...
describe('Segment Creation Bug Debug', () => {
  ...
  it('should test hasSegments check works properly', () => {
    ...
    if (segmentedRule) {
      ...
      const hasSegmentsBefore = (segmentedRule as any).hasSegments(context, worm);
      // console.log(`Before creation: hasSegments = ${hasSegmentsBefore}`);
      
      // Let the rule create segments naturally through sim.step()
      sim.step();
      
      // Check if hasSegments now returns true after natural creation
      const hasSegmentsFinal = (segmentedRule as any).hasSegments(context, worm);
