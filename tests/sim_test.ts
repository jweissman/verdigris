...
describe('Simulation basics', () => {
  it('should move a unit based on velocity', () => {
  ...
  it('should move multiple units independently', () => {
    const sim = new Simulator()
      .addUnit({ id: 'a', pos: { x: 0, y: 0 }, vel: { x: 1, y: 0 }, team: 'friendly', sprite: 'tiny', state: 'walk', hp: 10, mass: 1 })
      .addUnit({ id: 'b', pos: { x: 5, y: 0 }, vel: { x: -1, y: 0 }, team: 'hostile', sprite: 'worm', state: 'walk', hp: 5, mass: 1 });
  ...
  it('should not move units with zero velocity', () => {
    const sim = new Simulator()
      .addUnit({ id: 'c', pos: { x: 2, y: 2 }, vel: { x: 0, y: 0 }, team: 'friendly', sprite: 'rat', state: 'idle', hp: 3, mass: 1 });
    const next = sim.step();
  ...
  it('should accept input to change velocity', () => {
    const sim = new Simulator()
      .addUnit({ id: 'e', pos: { x: 0, y: 0 }, vel: { x: 0, y: 0 }, team: 'friendly', sprite: 'tiny', state: 'idle', hp: 10, mass: 1 });
    const input = { commands: { e: [{ action: 'move', target: { x: 1, y: 0 } }] } };
    sim.accept(input);
    expect(sim.units[0].vel.x).toBe(1);
  });
});
