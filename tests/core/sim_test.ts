...
describe('Simulation basics', () => {
  ...
  it('should move multiple units independently', () => {
    ...
    sim.addUnit({ id: 'a', pos: { x: 0, y: 0 }, intendedMove: { x: 1, y: 0 }});
  ...
  it('should not move units with zero intendedMove', () => {
    const sim = new Simulator();
    sim.addUnit({ id: 'c', pos: { x: 2, y: 2 }, intendedMove: { x: 0, y: 0 }});
    const next = sim.step();
  ...
  it('should cull unit if hp <= 0', () => {
    const sim = new Simulator();
    sim.addUnit({ id: 'd', pos: { x: 0, y: 0 }, intendedMove: { x: 0, y: 0 }, hp: 0});
    const next = sim.step();
